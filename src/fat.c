...
int search_path(int dev_id, char *path, unsigned short *start_cluster, unsigned short *chain_cluster)
{
  ...
  for(i = 0; i < o_length + 1 ; i++) {
    if (path[i] == FILE_SEPARATOR || path[i] == '\0') {
      ...
      found = find_name(dev_id, &path[offset_length], start_cluster, chain_cluster);

      offset_length= i + 1;
      DB_PRINTF("file found: %d, level:%d chain_cluster: %d\n",found, offset_length < o_length, *chain_cluster);

      if (found == -1 && offset_length < o_length  ) {
        return -20;
      } else {
...
int set_cwd(int dev_id, char *path)
{
  ...
  if (*path == '\0') {
    ...
    if (search_path(dev_id, path, &start_cluster, &chain_cluster)) {
      cwd = start_cluster;
    } else {
      cwd = 0;
    }
...
int fat_mkdir(int dev_id, char *path)
{
  ...
  DB_PRINTF("new start cluster:%d, chain_cluster:%d\n", start_cluster, chain_cluster);
  DB_PRINTF("found : %d\n", found);

  if (start_cluster > 0) {
    if (found ==-1) {
      chain_val = search_free_space (dev_id);
      chain_val = b_endian16(chain_val);
      write(dev_id, chain_cluster / BLOCK_SIZE + 1, (unsigned char*)&chain_val, chain_cluster % BLOCK_SIZE, sizeof(unsigned short));
      write(dev_id, b_endian16(chain_val) / BLOCK_SIZE + 1, (unsigned char*)&chain_last, b_endian16(chain_val) % BLOCK_SIZE, sizeof(unsigned short));
...
int fat_rmdir(int dev_id, char *path)
{
  ...
  unsigned short start_cluster = 0;
  unsigned short chain_cluster = 0;
  unsigned short free_chain_cluster = 0;
  unsigned short chain_val;
  unsigned short chain_last = 0xFFFF;
  int found = -1;
  int chain = 0;
  ...
  DB_PRINTF("del dir found :%d  cluster: start_cluster: %d , chain_cluster: %d\n", found, start_cluster, chain_cluster);



  if (search_free_dir(dev_id, start_cluster, &free_chain_cluster) > 64) { /* offset for two 0x2e entries . and .. */
...
int fat_first(int dev_id, directory *dir)
{
  ...
  DB_PRINTF("found :%d  cluster: start_cluster: %d, chain_cluster: %d\n", found, start_cluster, chain_cluster);
  ...
  if (start_cluster == 0) {
    do {
      ...
      found += sizeof(dent);
      read(dev_id, directory_start, (unsigned char*)&dent, found, sizeof(dent));
    } while (*dent.filename == 0xe5 && *dent.filename == 0x00);
  } else {
    chain = start_cluster;
    while (chain != 0xFFFF) {
      chain_cluster = chain;
...
exit:
  ...
  if (start_cluster !=0 && found == b_per_alloc * BLOCK_SIZE) {
    dir->dirent = 0;
  } else {
    dir->dirent = found;
  }
  dir->cluster = chain_cluster;
...
int fat_next(int dev_id, directory *dir)
{
  ...
  if (start_cluster == 0) {
    ...
    found += sizeof(dent);
    DB_PRINTF("-----------F:%d----chain:%d----------\n",found, chain);
    while (chain != 0xFFFF) {
      chain_cluster = chain;
      for(i = found / BLOCK_SIZE; i < b_per_alloc; i++) {
        for(j = found % BLOCK_SIZE; j < BLOCK_SIZE; j+= sizeof(dent)) {
          read(dev_id, directory_start + root_blocks + (chain / 2 - 2) + i, (unsigned char*)&dent, j, sizeof(dent));
          DB_PRINTF("%c%c%c\t",dent.filename[0],dent.filename[1],dent.filename[2]);
          DB_PRINTF("sub dir:%d\n",(chain / 2 - 2) + i);
          if (dent.filename[0] != 0x00 && dent.filename[0] != 0xe5) {
...
int fat_del(int dev_id, char* filename)
{
  ...
  DB_PRINTF("Dir Cluster: %d Chain Cluster: %d Cluster offset(bytes): %d\n",start_cluster, chain_cluster, found);

  if (start_cluster == 0) {
    read(dev_id, directory_start, (unsigned char*)&dent, found, sizeof(dent));
  } else {
    read(dev_id, directory_start + root_blocks + (chain_cluster / 2 - 2) * boot.b_per_alloc, (unsigned char*)&dent, found, sizeof(dent));
  }
  entry_cluster = l_endian16(dent.start_cluster);

  DB_PRINTF("name:%s\n",filename);
  DB_PRINTF("entry_cluster:%d\n",chain_cluster);
  ...
  if (start_cluster == 0) {
    ...
    write(dev_id, directory_start + root_blocks + (chain_cluster / 2 - 2) * boot.b_per_alloc, (unsigned char*)&dent, found, sizeof(dent));
  }
  return 0;
}
...
int fat_open(int dev_id, char* filename, int flags, int mode)
{
  ...
  */
  ...
  case O_RDONLY | O_TRUNC | O_CREAT:
    found = search_path(dev_id, filename, &start_cluster, &chain_cluster);
    DB_PRINTF("File offset: %d  Chain Cluster: %d\n",found, chain_cluster);
    if (found == -1) {
      /* read the root block */
      read(dev_id, 0, (unsigned char*)&boot, 0, sizeof(boot));
      ...
      if (start_cluster > 0) {
        if (found == -1) {
          ...
          found = search_free_dir(dev_id, b_endian16(chain_val), &chain_cluster);
          chain = b_endian16(chain_val);
          DB_PRINTF("new allocation chain cluster : %d start cluster: %d\n", chain, start_cluster);
          DB_PRINTF("new allocation found offset : %d\n", found);
        } else {
          chain = chain_cluster;
        }
        directory_start = directory_start + root_blocks + (chain / 2 - 2) * boot.b_per_alloc;
